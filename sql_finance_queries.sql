...
select sum(balance) as total_bal_checking from accounts where accounttype='checking';

-- 4. What is the total balance of all accounts associated with customers who live in Los Angeles?

select sum(a.balance) as los_angles_balance from  accounts a inner join customers c on c.customerid=a.customerid 
where city='Los Angeles';

-- 5. Which branch has the highest average account balance?

select b.branchname,b.city, b.branchid,avg(a.balance) as avg_balance from accounts a inner join  branches b on a.branchid=b.branchid
group by b.branchname ,b.city, b.branchid
order by  avg_balance desc
limit 1;

-- 6. Which customer has the highest current balance in their accounts?

select c.customerid ,c.firstname , c.lastname,sum(a.balance) as high_balance from customers  c inner join  accounts  a on  a.customerid=c.customerid 
...
select  a.customerid,c.firstname ,c.lastname ,count(t.accountid) as most_transaction from transactions t inner join accounts a on a.accountid=t.accountid join customers c on c.customerid=a.customerid
group by  c.firstname ,c.lastname,a.customerid
order by most_transaction desc;

-- 8.Which branch has the highest total balance across all of its accounts?

select b.branchid ,b.branchname ,b.city, sum(a.balance) as total_balance from branches b  inner join accounts a on b.branchid=a.branchid 
group by  b.branchname ,b.city ,b.branchid 
order by  total_balance desc;

-- 9. Which customer has the highest total balance across all of their accounts, including savings and checking accounts?

select c.customerid ,c.firstname ,c.lastname,sum(a.balance) as max_balance  from customers c inner join accounts a on  a.customerid=c.customerid 
...
select b.branchid ,b.branchname ,b.city , count(t.transactionid) as highest_transaction from accounts  a inner join transactions t on a.accountid=t.accountid join 
branches b on b.branchid=a.branchid
